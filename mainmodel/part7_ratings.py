...
def build_preseason_priors(div, season, all_df):
    ...
    # Use TM valuation-adjusted carry weights if available
    # Falls back to flat tier-based weights if not
    tm_carry = {}
    if os.path.exists("data/tm_valuations.csv"):
        try:
            val_df = pd.read_csv("data/tm_valuations.csv")
            val_df = val_df[val_df["season"] == season]
            for _, row in val_df.iterrows():
                if pd.notna(row.get("carry_weight_adjusted")):
                    tm_carry[row["Team"]] = float(row["carry_weight_adjusted"])
        except Exception:
            pass

    base_carry = CARRY_WEIGHT_TIER1 if div in TIER1_DIVS else CARRY_WEIGHT_TIER2
    priors     = {}

    for team in returning:
        carry = tm_carry.get(team, base_carry)
...
def fit_ratings(subset, half_life=HALF_LIFE, market_weight=MARKET_WEIGHT,
    ...
    # Weather control — extreme conditions make goal diff less informative.
    # Scale actual goal diff rows down by weather_adj (1.0 = normal, 0.85 = hot/windy).
    # Market odds rows are NOT scaled — bookmakers already price in weather.
    # Sources: Pavlinovic 2024 (temp), Zhong 2024 (wind), Bray 2008 (wind threshold).
...
def build_ratings_table(div, season=CURRENT_SEASON):
    ...
    return table, home_adv, res.get("attack", {}), res.get("defence", {})


# ── 6. SAVE RATINGS JSON ────────────────────────────────────────────────────
def save_ratings_json(all_tables, path="data/ratings.json"):
    """Export ratings to JSON for the web dashboard."""
    import json
    out = {}
    for div, (table, home_adv, attack, defence) in all_tables.items():
        teams = []
        for idx, row in table.iterrows():
            t = row["Team"]
            teams.append({
                "rank": int(idx),
                "team": t,
                "overall": round(row["Rating"], 3),
                "attack": round(attack.get(t, 0.0), 3),
                "defence": round(defence.get(t, 0.0), 3),
                "win_pct": round(row["Win%"], 1),
                "gp": int(row["GP"]),
            })
        out[div] = {
...
def main(divs=None, season=CURRENT_SEASON):
    ...
    # Save ratings JSON for web dashboard
    if all_tables:
        save_ratings_json(all_tables)
